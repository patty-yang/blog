...
| -------------- | ------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------- | ---------------------------------------- |
| **兼容性**     | ✅ 所有浏览器支持<br>🕰️ 最早的客户端存储方案                                                                             | 🆕 HTML5 新增特性                        | 🆕 HTML5 新增特性                        |
| **浏览器行为** | 📥 响应头含 set-cookie 时自动保存<br>📤 请求时自动附带到请求头<br>🔐 常用于维持用户登录状态<br>⚠️ 可能导致 CSRF 攻击风险 | 🚫 无默认行为<br>💾 持久性存储           | 🚫 无默认行为<br>⏳ 会话级存储           |
| **存储限制**   | 📏 同域下总量限制约 4MB<br>🔗 与 domain、path 关联                                                                       | 📈 无存储上限限制<br>🔗 仅与 domain 关联 | 📈 无存储上限限制<br>🔗 仅与 domain 关联 |

## 🔐 加密算法

| 加密类型                              | 加密方式                     | 特点                                                           |
| ------------------------------------- | ---------------------------- | -------------------------------------------------------------- |
| 对称加密<br>(Symmetric Encryption)    | 使用同一个密钥进行加密和解密 | • 加密解密速度快<br>• 适合大量数据加密<br>• 密钥管理是主要挑战 |
| 非对称加密<br>(Asymmetric Encryption) | 使用公钥加密、私钥解密的方式 | • 安全性更高<br>• 计算复杂度大<br>• 适合小数据量加密           |

## 输入 URL 之后会发生什么

1. 自动补全协议和端口
2. 自动完成 URL 编码
3. 会根据 URL 地址查找本地缓存，根据缓存规则看是否命中缓存，如果命中缓存的话直接使用缓存
4. 通过 DNS 解析查找到服务器的地址
5. 浏览器向服务器发送 TCP 连接的申请，完成三次握手后(类似于打电话 确认双方都能听到)，建立通道。
6. 如果使用了 HTTPS 协议，还会进行 SSL 握手，建立加密通道。使用 SSL 握手时，会确定是否使用 HTTP2
7. 浏览器决定要附带那些 cookie 到请求头中
8. 浏览器自动设置好请求头、协议版本、cookie、发送 get 请求
9. 服务器处理，等服务器响应 HTTP 报文给浏览器
10. 浏览器根据使用的协议版本，以及 connection 字段的约定，决定是否要保留 TCP 连接
11. 浏览器根据响应状态嘛来决定如何处理这次的响应
12. 浏览器根据响应头中的 content-type 字段识别响应类型，如果是 text/html 则对响应进行 HTML 解析
...
15. 在解析过程中还会触发一系列的事件，当 DOM 树完成后会触发 DOMContentLoaded 事件，当所有资源加载完成后会触发 loaded 事件
...
| 分类           | 说明                                                                                              |
| -------------- | ------------------------------------------------------------------------------------------------- |
| 📝 定义        | Monorepo 是将多个项目代码统一存储在同一个代码仓库中的开发策略，而不是将每个项目分散在不同的仓库中 |
| 🌟 主要优势    |                                                                                                   |
| 代码共享与复用 | - 便捷的依赖代码共享<br>- 统一依赖管理<br>- 易于抽取和维护公共模块                                |
| 依赖管理       | - 统一版本控制<br>- 避免依赖冲突<br>- 简化依赖更新流程                                            |
| 工程化管理     | - 统一构建流程<br>- 统一代码规范<br>- 统一测试策略                                                |
| 💡 使用场景    | - 大型项目开发<br>- 多项目间需共享代码和资源<br>- 需统一管理和维护的项目群                        |

```shell
pnpm install 要安装的包名 --filter 安装到的位置 --workspace
